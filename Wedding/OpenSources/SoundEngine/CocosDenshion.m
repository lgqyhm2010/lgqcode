	...
	for (int i=0; i < _channelTotal; i++) {
		...
		if (_buffers[soundId] == _sourceBufferAttachments[i]) {
			...
			if (state == AL_PLAYING) {
				...
				alSourcei(_sources[i], AL_LOOPING, NO);//Turn off looping otherwise loops will never end
 ...
 * @return YES if buffer loaded okay for play back otherwise false
 */
...
- (ALuint)playSound:(int) soundId channelGroupId:(int)channelGroupId pitch:(float) pitch pan:(float) pan gain:(float) gain loop:(BOOL) loop {
	...
	if (channel != CD_CHANNEL_GROUP_NON_INTERRUPTIBLE) {
		...
		return [self _startSound:soundId channelId:channel pitchVal:pitch panVal:pan gainVal:gain looping:loop checkState:YES];
	} else {
		...
		if (channel != CD_CHANNEL_GROUP_NON_INTERRUPTIBLE) {
			...
			return [self _startSound:soundId channelId:channel pitchVal:pitch panVal:pan gainVal:gain looping:loop checkState:NO];
		} else {
